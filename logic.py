...
def calculate_variables(matrix, vars):
    ...
    for row in range(n):
        ...
        simplificat = simplify(solution)
        if " / " in simplificat:
            left,right = simplificat.split("/")
            variables_dict[main_var] = {"numarator":left.strip(),"numitor":right.strip()}
        else:
            variables_dict[main_var] = {"numarator":simplificat,"numitor":"1"}
    ...
    var_type_pos = main_vars
    if sec_index != 0:
        print(f"Variabilele secundare: {', '.join(sec_vars)}")
        var_type_neg = sec_vars
